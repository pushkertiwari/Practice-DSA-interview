...
class Coupon {
    constructor() {
        if (this.constructor === Coupon) {
            throw new Error("Abstract class 'Coupon' cannot be instantiated directly.");
        }
    }
...
class CouponNext extends Coupon {
    ...
    apply(products, startIndex = 0) {
        for (let i = startIndex; i < products.length; i++) {
            if (!products[i].discounted) { // Check if the product is already discounted
...
class CouponNextType extends Coupon {
    ...
    apply(products) {
        for (let i = 0; i < products.length; i++) {
            if (products[i].type === this.type && !products[i].discounted) { // Check type and if not discounted
                products[i].price *= (1 - (this.percentage / 100));
                products[i].discounted = true; // Mark as discounted
                break;
            }
...
cart.addItem(new CouponNext(5)); // 5% off next item
cart.addItem(new Product(24.99, ProductType.TShirt));
